...
% 
% % 预分配内存
% total_grid = length(Omega_list)*length(a_list)*length(i_list)*...
%              length(e_list)*length(omega_list)*length(M_list);
% orb_cell = cell(total_grid, 1);
% total_idx = 0;
% 
...
%                         final_t =0;
%                         tem = zeros(1, 30);
%                         for it = 1:1:length(pos_re)
%                             for jd = 1:1:345
%                                 if pos_re(it,jd)<30000 && vol_re(it,jd)<150
%                                     count = count+1; % 捕获数
%                                     tem(8+count) = jd;   % 记录哪个碎片被捕获
%                                     final_t = it;     % 最后一个碎片的捕获时刻
%                                 end
%                             end
%                         end
% 
