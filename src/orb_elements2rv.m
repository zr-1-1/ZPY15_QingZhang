function state = orb_elements2rv(elements)
...
%                  omega - 近地点幅角 [rad]
%                  M   - 平近点角 [rad]
%   全局参数：mu - 引力常数 [m^3/s^2]，调用前须初始化。
%   输出：
%       state    - Nx6 矩阵，每行为 [x, y, z, vx, vy, vz]
%                  位置单位为 m，速度单位为 m/s（ECI惯性系）
%
%   示例：
%       global mu;
%       mu = 3.986004418e14; % m^3/s^2
%       elem = [42164000, 0.001, 0.1, 0.5, 0.3, 1.2]; % a 为 m，角度为 rad
%       state = orb_elements2rv(elem);
%       fprintf('位置 (m): [%.3f, %.3f, %.3f]\n', state(1:3));
%       fprintf('速度 (m/s): [%.6f, %.6f, %.6f]\n', state(4:6));
    ...
    for k = 1:N
        ...
        v_PQW = sqrt(mu/p) * [-sinf; e(k)+cosf; 0];   % 单位 m/s
