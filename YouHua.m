...
%% -------- 2.5 ★ 关键：检测 times 语义 --------
fprintf('\n===== capture_times 诊断 =====\n');
t_range = zeros(n_win, 2);
for ww = 1:n_win
    t_all = [];
    for i = 1:numel(precalc{ww})
        if ~isempty(precalc{ww}{i}.times)
            t_all = [t_all; precalc{ww}{i}.times];
        end
    end
    if ~isempty(t_all)
        t_range(ww,:) = [min(t_all), max(t_all)];
        fprintf('  窗%d: capture_times 范围 [%.2fh, %.2fh]\n', ...
            ww, min(t_all)/3600, max(t_all)/3600);
    end
end

% 判断依据：
%   相对时间：每个窗的 times 都在 [0, 28800] 范围内
%   绝对时间：窗2~4 的 times 最小值会 > 5h
if n_win >= 2 && t_range(2,1) > 5*3600
    TIMES_MODE = 'absolute';
    fprintf('  → 判定：capture_times 是绝对时间\n');
elseif n_win >= 2 && t_range(2,2) <= 8.5*3600
    TIMES_MODE = 'relative';
    fprintf('  → 判定：capture_times 是相对窗口起点\n');
else
    TIMES_MODE = 'absolute';   % 默认绝对
    fprintf('  → 判定：默认绝对时间\n');
end
...
for k = 1:n_sc
    ...
    for ww = 1:n_win
        ...
        % ★ 关键：根据模式计算绝对时刻
        if strcmp(TIMES_MODE, 'absolute')
            times_abs = times;
        else
            times_abs = window_start_t(ww) + times;
        end
...
save('combine_result_chain_v3.mat', 'best_sol', 'all_deb_global', ...
     'best_global_obj', 'TIMES_MODE', '-v7.3');
